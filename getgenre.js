...
const CLIENT_ID = '4d947e2d5e2b469aa1e5bd0e7185a25b';
const CLIENT_SECRET = '8713ec5b97894fef8208bbd119354824';
...
app.get('/login', (req, res) => {
    const scopes = 'user-read-currently-playing'; // Add any additional scopes as needed
...
app.get('/callback', (req, res) => {
    ...
    request.post(authOptions, (error, response, body) => {
        if (!error && response.statusCode === 200) {
            ...
            // Periodically check for changes in the currently playing track
            setInterval(() => {
                checkCurrentlyPlaying(access_token);
            }, 5000); // Check every 5 seconds initially
        } else {
            res.status(response.statusCode).send("Error while authenticating with Spotify.");
        }
    });
});
...
// Simplifies sub-genres into main genres
